 ...
 */
export const getDatabase = (state: any, pageNumber: number, reportId: number) => {
  if (state == undefined || pageNumber == undefined || reportId == undefined) {
    // TODO - use DMBS default database instead of neo4j.
    return 'neo4j';
  }
  if (
    state.dashboard.pages[pageNumber] == undefined ||
    state.dashboard.pages[pageNumber].reports.find((o) => o.id === reportId) == undefined
  ) {
    // TODO - use DMBS default database instead of neo4j.
    return 'neo4j';
  }
  const reportDatabase = state.dashboard.pages[pageNumber].reports.find((o) => o.id === reportId).database;
