...
/**
 * Experimental - A live time chart that is updated with a new value each time the report is run.
 * Should return a pair of values only.
 */
class NeoLiveTimeChartReport extends NeoReport {
    ...
    componentDidMount() {
        ...
        var height = -150    + props.height * 100 - yShift;
        var margin = {top: 0, right: 0, bottom: 20+yShift, left: xShift};
        ...
        svg.append("g")
            ...
            .call(d3.axisBottom(x)
                .tickFormat(d3.timeFormat("%H:%m:%S")).tickValues(data.map(function(data) { return new Date(data[0])}) ).ticks(d3.timeSecond.every(4)))
            .selectAll("text")
        ...
        svg.selectAll(".dot")
            ...
            .on("mousemove", function (d) {
                ...
                d3.select(".chart-tooltip")
                    ...
                    .html(prop3 + ": " + d[0] + "<br>" + prop2 + ": " + d[1] + "<br>");
            })
...
export default (NeoLiveTimeChartReport);
